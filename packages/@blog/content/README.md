...
`registries.diagramNames` · `og.palette` 가 필수(`ContentValues` 계약)이고,
...
| `og`         | `palette`가 **필수**(satori는 CSS 변수를 못 읽어 리터럴 색이 필요하다 — 소비자가 자기 디자인 토큰에서 해석해 넘긴다). `width` · `height`는 소셜 카드 표준이라 기본값                                                                                                 |
