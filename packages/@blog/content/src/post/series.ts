...
export interface SeriesReader {
  ...
  isSeriesFolder: (seriesName: string, meta?: SeriesMeta | null) => boolean;
}

export interface SeriesReaderDeps {
  /** 마크다운 원본 디렉터리 절대 경로 — repository와 **같은** 값이어야 한다 */
  postsDir: string;
  /** dev면 캐시를 우회해 `_series.yml` 생성/삭제가 즉시 반영된다 */
  isDevelopment: () => boolean;
}

/**
 * 시리즈 선언(`_series.yml`) 리더 factory. 캐시는 인스턴스(클로저) 안에 산다 —
 * 루트가 다른 인스턴스끼리 캐시가 섞이지 않는다.
 */
export function createSeriesReader(deps: SeriesReaderDeps): SeriesReader {
  const { postsDir, isDevelopment } = deps;
  const metaCache = new Map<string, SeriesMeta | null>();

  function getSeriesMeta(seriesName: string): SeriesMeta | null {
    if (!isDevelopment() && metaCache.has(seriesName)) {
      return metaCache.get(seriesName) ?? null;
    }

    // 시리즈 폴더 자체가 없다 = 경로 불일치다. seriesName은 실제 폴더 스캔
    // (repository.ts)에서 온 값이라, 폴더가 안 보이면 "시리즈가 아님"이 아니라
    // 이쪽의 postsDir가 다른 곳을 보고 있다는 뜻이다. 예전에는 이 경우에도
    ...
    // 수 있어야 한다")가 잠근다.
    const seriesDir = join(postsDir, seriesName);
    if (!existsSync(seriesDir)) {
      console.warn(
        `[series] 시리즈 폴더를 찾을 수 없습니다: ${seriesDir} — ` +
          `defineContent의 root/dirs.content가 실제 posts 위치와 어긋났거나, ` +
          `실제 스캔에 없는 폴더 이름을 직접 조회한 것일 수 있습니다(테스트 등).`,
      );
      metaCache.set(seriesName, null);
      return null;
    }
   ...
   *
   * `meta` 인자로 디스크 접근을 주입할 수 있다 — 스크립트의 단위 테스트가 실제
   * posts/ 폴더 상태에 따라 흔들리지 않도록. `undefined`(미지정 → 디스크 조회)와
   * `null`(메타 없음)이 여기서 구분된다.
   */
  function isSeriesFolder(
    seriesName: string,
