...
| 패키지명                      | 종류                 | 비고                                                                                                                                                                                                                                                                                                                                                                   |
| ----------------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@blog/analytics`             | 📈 조회수·대시보드   | 순수 계산(개요·파생 통계)·계약·저장소 팩토리·`database.types.ts`. **클라이언트를 만들지 않고 주입받는다** — `@supabase/*`는 타입으로만 쓴다. 블로그 앱에서 떼어 냈고(그 2,560줄에 `react`·`next/` import가 0이었다), 앱에는 클라이언트를 만드는 배선만 남았다.                                                                                                         |
| `@blog/content`               | 📚 콘텐츠 프레임워크 | 블로그의 스키마·로더·공개 판정·URL 계약·빌드 스크립트·2층 검증(`validate-posts` 원문 / `check-seo` 산출 HTML). 문 셋(`@blog/content`, `@blog/content/seo`, 클라이언트용 `@blog/content/urls`) + 빌드 스크립트를 내놓는 `blog-content` bin, **소스 익스포트(빌드 스텝 없음 — node가 `.ts`를 그대로 실행)**. 자세히는 [패키지 README](packages/@blog/content/README.md). |
