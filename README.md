...
| **Renovate**               | 의존성 자동 업데이트(루트 `renovate.json`). minor/patch는 CI만 보고 자동 머지(dependencies·devDependencies·catalog·overrides·`@types/*`·GitHub Actions 모두, npm과 액션은 공개 3일 뒤) — major만 `deps-major` 라벨을 달고 사람이 본다. 예외로 `anthropics/claude-code-action`(SHA 고정)과 월간 lock file maintenance는 사람이 머지한다. |
...
| `preview-blog.yml`          | `pull_request`(블로그 경로)                                                  | 빌드(시크릿 없는 잡) → `wrangler versions upload`(토큰을 쥔 잡, 아티팩트만 받는다) → 프리뷰 URL을 PR에 코멘트                                                                                                                                                                                                                                                                   |
...
| `claude-code-review.yml`    | PR opened/synchronize (봇 PR은 `deps-major` 라벨일 때만)                     | PR 자동 코드 리뷰. 판정은 PR의 👍 리액션이다 — 새 실행이 시작되면 이전 👍를 떼고, 그 실행이 실제로 게시한 요약 코멘트에 critical·high 지적이 0건일 때만 다시 붙인다(fail-closed)                                                                                                                                                                                                |
