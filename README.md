...
| `@blog/content`               | 📚 콘텐츠 프레임워크 | 블로그의 스키마·로더·공개 판정·URL 계약·빌드 스크립트·2층 검증(`validate-posts` 원문 / `check-seo` 산출 HTML). 문 넷(`@blog/content`, `@blog/content/seo`, 클라이언트용 `@blog/content/urls`·`@blog/content/dates`) + 빌드 스크립트를 내놓는 `blog-content` bin, **소스 익스포트(빌드 스텝 없음 — node가 `.ts`를 그대로 실행)**. 자세히는 [패키지 README](packages/@blog/content/README.md). |
