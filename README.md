...
| **🔬 같은 사이트, 다른 프레임워크** | `apps/blog/web-svelte`                                                      | 같은 원고·같은 콘텐츠 패키지를 SvelteKit으로 한 번 더 구현해 번들 크기·저작 DX·프레임워크를 나란히 재는 실험([#392](https://github.com/Han5991/fe-lab/issues/392)). 프리뷰 URL로만 뜨고 운영을 건드리지 않는다. |
...
| `blog/web-svelte` | `@blog/web-svelte`  | 🔬 블로그 (병행 재구현) | SvelteKit 2 + Svelte 5 runes (`adapter-static`), Vite 8, Panda CSS, Supabase      | **같은 사이트를 SvelteKit으로 한 번 더 짓는 실험**([#392](https://github.com/Han5991/fe-lab/issues/392)). 운영 블로그를 대체하지 않는다 — 나란히 두고 번들·저작 DX를 잰다. 원고·`@blog/content`·`@blog/site-values`·`@blog/analytics`·`@blog/diagram`은 React 판과 공유. 배포는 **프리뷰 URL뿐**(전용 Worker, 도메인 없음), Supabase는 로컬만. |
...
| 패키지명                      | 종류                 | 비고                                                                                                                                                                                                                                                                                                                                                                     |
| ----------------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `@blog/analytics`             | 📈 조회수·대시보드   | 순수 계산(개요·파생 통계)·계약·저장소 팩토리·`database.types.ts`. **클라이언트를 만들지 않고 주입받는다** — `@supabase/*`는 타입으로만 쓴다. 블로그 앱에서 떼어 냈고(그 2,560줄에 `react`·`next/` import가 0이었다), 앱에는 클라이언트를 만드는 배선만 남았다. 자세히는 [패키지 README](packages/@blog/analytics/README.md).                                             |
| `@blog/content`               | 📚 콘텐츠 프레임워크 | 블로그의 스키마·로더·공개 판정·URL 계약·빌드 스크립트·2층 검증(`validate-posts` 원문 / `check-seo` 산출 HTML). 문 셋(`@blog/content`, `@blog/content/seo`, 클라이언트용 `@blog/content/client`) + 빌드 스크립트를 내놓는 `blog-content` bin, **소스 익스포트(빌드 스텝 없음 — node가 `.ts`를 그대로 실행)**. 자세히는 [패키지 README](packages/@blog/content/README.md). |
| `@blog/diagram`               | 📐 다이어그램 좌표   | 선언형 다이어그램의 좌표 계산기 — 프레임워크를 모르고 숫자만 낸다. 그리는 일은 앱의 몫. React 판과 SvelteKit 판이 같은 좌표를 쓰므로 같은 원고가 두 사이트에서 픽셀 단위로 같은 그림이 된다.                                                                                                                                                                             |
| `@blog/site-values`           | 🪪 사이트 값         | 사이트 정체성 리터럴(이름·URL·저자·타임존…)의 단일 출처. 값을 import하지 않는 순수 리터럴이고, 두 블로그 앱이 함께 읽는다 — 사본을 두면 한쪽만 고쳤을 때 두 산출물이 **조용히** 갈린다. 번들 규칙은 프레임워크 어휘라 여기 없고 앱이 각자 선언한다.                                                                                                                      |
...
| **Cloudflare Workers (Svelte)** | SvelteKit 판 전용 Worker `blog-svelte`(`apps/blog/web-svelte/wrangler.jsonc`) — **프리뷰 URL 전용**이다. `routes`가 없어 커스텀 도메인도 프로덕션 배포 경로도 없고, `_headers`가 전 경로에 `X-Robots-Tag: noindex`를 건다 |
...
| 워크플로                    | 트리거                                                                       | 하는 일                                                                                                                                                                                                                                                                                                                                            |
| --------------------------- | ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ci.yml`                    | `pull_request`, `push: main`                                                 | quality-checks(lint·types·test·frontmatter·포맷) + **블로그 두 판**을 빌드해 산출물 검사까지 — SvelteKit 판 게이트가 **항상** 도는 자리다(`preview-blog.yml`도 같은 빌드를 부르지만 포크 PR·비블로그 PR, 자기 앱 경로를 안 건드린 PR에는 안 돈다). `build-blog-svelte` 입력으로 끄며 `deploy-blog.yml`이 끈다. Next 빌드 캐시 복원 포함            |
...
| `preview-blog.yml`          | `pull_request`(블로그 경로·이 파일)                                          | **앱 둘을 매트릭스로** 빌드 → `wrangler versions upload` → 프리뷰 URL을 PR에 코멘트(앱마다 자기 Worker·자기 sticky 코멘트). 줄마다 **자기 앱 디렉터리나 공유 경로**(원고·`packages/@blog`·이 파일)가 바뀐 PR에서만 빌드한다. 체크 이름은 `preview`(Next.js)·`preview-svelte`로 못 박는다 — 매트릭스가 이름을 바꾸면 required check가 영영 대기한다 |
