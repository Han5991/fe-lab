...
| `deploy-blog.yml`           | `push: main`(블로그 경로), cron `0 0 * * *`, dispatch                        | quality-checks → `--force` 빌드(turbo 캐시를 읽지 않는다 — 빌드 결과가 빌드 시각에 달려 있다) → `/posts/` 프리렌더 링크 검증 → Workers 배포. 빌드 스텝이 넣는 env는 `NEXT_PUBLIC_PR_COUNT`·`NODE_ENV` 둘뿐이고 나머지 `NEXT_PUBLIC_*`은 커밋된 `.env.production`에서 온다 |
