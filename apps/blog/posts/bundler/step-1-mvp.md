...
### 1) Live Binding (라이브 바인딩): 실시간 CCTV vs 스냅샷

ESM의 가장 독특한 점은 값을 **참조(Reference)**로 연결한다는 것입니다.

- **CJS (값 복사):** `require`는 실행 결과값을 복사해서 가져옵니다. 나중에 원본 모듈의 값이 바뀌어도 가져온 값은 변하지 않습니다. (스냅샷)
- **ESM (라이브 바인딩):** `import`는 값이 들어있는 메모리 주소 자체를 가리킵니다. 원본 모듈에서 값이 바뀌면 가져다 쓰는 쪽에서도 즉시 반영됩니다. (실시간 CCTV)

### 2) 왜 이렇게 만들었을까? (순환 참조 문제)

CJS가 훨씬 직관적인데 왜 굳이 이런 복잡한 방식을 택했을까요? 가장 큰 이유는 **순환 의존성(Circular Dependency)** 해결입니다.

모듈 A가 B를 부르고, B가 다시 A를 부르는 상황에서 CJS는 "아직 실행이 덜 끝난 A" 대신 빈 객체(`{}`)를 넘겨버려 런타임 에러를 발생시키는 경우가 많습니다. 반면 ESM은 **"평가(Parsing)"** 단계에서 이름표만 미리 붙여두고, **"실행(Execution)"** 시점에 값을 채워 넣는 라이브 바인딩 방식을 통해 이런 닭-알 문제를 우아하게 해결합니다.
