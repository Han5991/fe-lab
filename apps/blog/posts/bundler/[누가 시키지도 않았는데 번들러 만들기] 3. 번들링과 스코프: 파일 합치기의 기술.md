...
const { plus } = require(1); // './lib/index.js'가 ID: 1로 변환됨
```
...
// 1. 가져올 변수들 문자열 만들기: "{ plus }"
const imported = node.specifiers.map(...).join(', ');

// 2. require 문 만들기: "const { plus } = require(1);"
const newCode = `const { ${imported} } = require(${moduleId});`;

// 3. 덮어쓰기! (수술 집도)
// 원본 코드의 start(0)부터 end(38)까지를 newCode로 교체합니다.
...
### ⚠️ 현재 구현의 한계와 브라우저 사용법

우리가 만든 번들러는 **Node.js의 CommonJS 시스템**을 런타임으로 모방하고 있습니다. 따라서 생성된 `dist/index.mjs` 파일도 Node.js 환경(`module` 패키지 의존)을 전제로 합니다.

브라우저에서 이 라이브러리를 테스트하려면 `script type="module"` 대신, `dist/index.js`를 로드하여 전역 변수(`window.BundlerLibrary`)를 사용하는 방식을 써야 합니다. 이를 위해 `Graph.ts`의 런타임 코드에 전역 변수 할당 로직을 추가했습니다.

👉 **[Graph.ts 런타임 구현 소스 코드 보기](https://github.com/Han5991/fe-lab/blob/main/packages/@package/bundler/src/Graph.ts)**

현대적인 번들러(Vite, Rollup)들은 브라우저를 위한 순수 ESM 번들링을 지원하지만, 우리는 **모듈 시스템의 원리** 를 깊이 이해하기 위해 이러한 방식을 선택했습니다.

---

## 7. [Tip] CJS와 ESM을 동시에?

참고로, 우리가 만든 번들러는 CJS 포맷(`dist/index.js`)뿐만 아니라 **ESM 포맷(`dist/index.mjs`)** 도 함께 생성합니다.

최신 라이브러리들은 두 환경을 모두 지원하는 **Dual Package** 전략을 사용하기 때문입니다. 다만, ESM 생성 로직은 CJS 번들 결과를 변수에 담아 다시 `export` 하는 방식을 사용했습니다. (자세한 배포 전략은 **Step 5**에서 다룹니다!)

---

## 8. [Next Step] 하지만 이게 끝이 아닙니다
