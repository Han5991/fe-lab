...
지난 시간([Step 2. 코드를 데이터로 보는 법](https://blog.sangwook.dev/posts/no-one-asked-library-bundler-02-ast-graph/)), 우리는 코드를 **데이터(AST)**로 변환하고, 파일들 사이의 복잡한 관계를 **그래프(Graph)**로 그려냈습니다. 이제 우리 손에는 프로젝트의 모든 파일 정보가 담긴 '지도'가 쥐어져 있습니다.

이제 남은 건 하나입니다. 이 흩어진 파일 조각들을 모아서, 브라우저가 실행할 수 있는 **단 하나의 결과물(`bundle.js`)**로 만드는 것입니다.

---

## 1. [Why] 그냥 텍스트로 합치면 안 되나요?

가장 단순하게 생각해보죠. 번들링이란 결국 파일을 합치는 것이니, 파일 내용을 순서대로 복사해서 붙여넣으면(Concatenate) 되지 않을까요?
...
만약 위 코드를 하나의 파일로 합친다면, 브라우저는 즉시 `SyntaxError: Identifier 'name' has already been declared`를 뱉어낼 것입니다. 파일이 나뉘어 있을 때는 각자의 공간(Scope)이 있었지만, 하나로 합쳐지는 순간 **전역 스코프(Global Scope)**라는 거대한 운동장에서 이름 쟁탈전이 벌어지기 때문입니다.

이것이 바로 번들러가 단순히 파일을 이어 붙이는 `cat` 명령어와 다른 점입니다. 번들러의 첫 번째 임무는 **"각 모듈의 독립적인 방(Scope)을 지켜주는 것"**입니다.

---

## 2. [Solution] 함수로 감싸기 (IIFE)

이 문제를 해결하기 위해 우리의 미니 번들러는 **IIFE(즉시 실행 함수)** 패턴을 사용합니다. 자바스크립트에서 스코프(Scope)를 생성하는 가장 전통적이고 확실한 방법은 **함수**이기 때문입니다.
...
>
> 최근의 **Rollup**, **Vite(Rolldown)**, **Webpack 5** 등은 성능을 위해 **Scope Hoisting(스코프 호이스팅)**이라는 기술을 사용합니다. 함수로 감싸지 않고 최대한 하나의 스코프로 합치되, 변수 이름이 겹치면 자동으로 이름을 바꿔주는(Renaming) 방식이죠.
>
> 하지만 우리는 번들러의 원리인 **"모듈 시스템의 구현"**을 배우는 것이 목표이므로, 모듈의 경계가 명확히 보이는 이 방식을 채택하여 직접 구현해 보겠습니다.

---

## 3. [Challenge] 브라우저엔 require가 없다
...
따라서 번들러는 단순히 파일을 합치는 것뿐만 아니라, **Node.js의 모듈 시스템(`require`, `module`, `exports`)을 브라우저에서 흉내 내는 코드(Runtime Shim)**를 직접 주입해 줘야 합니다.
...
### 🔑 핵심: 왜 캐시(Cache)가 필요한가요?

위 코드에서 `cache[id]`를 확인하는 로직은 선택이 아니라 **필수**입니다. 두 가지 중요한 이유가 있습니다.

1.  **효율성 & 싱글톤(Singleton)**: 모듈은 여러 곳에서 `import` 되더라도 **단 한 번만 실행**되어야 합니다. 그렇지 않으면 `count` 같은 상태 변수가 import 할 때마다 초기화되어 버립니다.
2.  **순환 참조 방지(Circular Dependency)**: `A -> B -> A` 구조에서 캐시가 없다면, `require`가 무한히 호출되어 **Stack Overflow**가 발생합니다. 캐시가 있으면, 두 번째 `A`를 불렀을 때 이미 만들어진(비어있더라도) 객체를 반환하여 루프를 끊을 수 있습니다.
...
## 5. [Deep Dive] AST 수술 집도하기: `import`가 `require`로 바뀌는 순간
...
`Module.transform()` 메서드가 실행되면, AST 트리를 순회하며 노드의 타입에 따라 수술(변환)을 집도합니다. 이 과정을 다이어그램으로 보면 다음과 같습니다.
...
타입을 확인하고(`ImportDeclaration`), 매핑된 ID(`1`)를 가져왔다면, 이제 `magic-string`이라는 메스를 들 차례입니다. 원본 코드를 건드리지 않고, 기존 `import` 문을 덮어쓰기(overwrite) 합니다.
...
## 6. [Bonus] 결과물의 형태: Dual Package 전략
...
## 7. [Next Step] 하지만 이게 끝이 아닙니다

이제 파일은 합쳐졌고, 브라우저에서도 돌아갑니다. (`bundle.js`)
하지만 **"배포 가능한 라이브러리"**가 되려면 아직 해결해야 할 치명적인 문제들이 남아있습니다.

1.  **디버깅의 악몽**: `bundle.js` 1번째 줄에서 에러가 났다고 하면, 원본 `src/index.js`의 몇 번째 줄인지 어떻게 알까요? (소스맵 필요)
2.  **무거운 짐**: 만약 내 라이브러리가 `react`를 쓴다면, 번들 파일 안에 리액트 소스코드 전체가 포함되어야 할까요? (Externals 필요)
3.  **진입점 설정**: 사용자가 내 라이브러리를 `import` 할 때와 `require` 할 때, 각각 다른 파일을 주려면 `package.json`을 어떻게 설정해야 할까요?

다음 편, **[Step 4. 소스맵과 배포 준비]**에서는 이 마지막 퍼즐 조각들을 맞추고, 우리가 만든 번들러로 실제 라이브러리를 배포할 준비를 마칩니다.
