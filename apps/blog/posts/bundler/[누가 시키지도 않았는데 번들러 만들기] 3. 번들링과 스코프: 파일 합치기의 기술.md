...
seoTitle: '[번들러] 3. 번들링과 스코프: 파일 합치기의 기술'
date: '2026-02-23'
status: published
slug: 'no-one-asked-library-bundler-03-bundling-scope'
excerpt: '단순히 파일을 이어붙이는 것과 번들링은 다릅니다. 같은 이름의 변수가 서로를 덮어쓰지 않도록 IIFE로 스코프를 가두는 전략과, 합쳐진 결과물이 브라우저에서 제대로 실행되게 만드는 런타임 구현 과정을 깊이 있게 다룹니다.'
...
**처참한 실패입니다.**
파일이 나뉘어 있을 때는 각자의 스코프가 보장되었지만, 하나로 합쳐지는 순간 **모든 변수가 전역 스코프에 노출되면서 충돌이 발생**하기 때문입니다.  
...
우리는 이 함수들을 **객체**에 담고, 그 위에서 **관리자**가 하나씩 꺼내 쓰는 구조를 만들 겁니다.
...
이제 우리의 목표가 명확해졌습니다. 위쪽의 **`require` 함수**를 구현하고, 아래쪽의 **모듈**을 채워 넣으면 됩니다.
...
이게 핵심입니다. 모듈은 `import` 할 때마다 실행되는 게 아닙니다. **최초 한 번만** 실행되고, 그 뒤로는 저장된 값을 써야 합니다.  
약 모듈이 호출될 때마다 매번 새로 실행된다면, 전역 설정이나 상태값들이 계속 초기화되어 버립니다. `import`는 **싱글톤**처럼 한 번 만든 객체를 계속 돌려써야 합니다.
...
> **[실제 런타임 구현 코드 보기 (Graph.ts:110)](https://github.com/Han5991/fe-lab/blob/1707765c3eb6249234463033335d08927ca25a13/packages/@package/bundler/src/Graph.ts#L110)**
...
우리가 작성한 코드는 최신 **ESM(`import/export`)** 이지만, 방금 만든 런타임은 **CJS(`require/exports`)** 기반입니다. 번들러는 코드를 합치기 전에, **`import` 문을 `require` 함수 호출로 바꿔치기** 해야 합니다.  
...
// Output (CJS) — default는 빼고, 이 모듈이 이미 내보낸 이름은 덮지 않는다
const _star_0 = require(1);
for (const key in _star_0) {
  if (key !== 'default' && !Object.prototype.hasOwnProperty.call(exports, key)) {
    exports[key] = _star_0[key];
  }
}
```
...
> **[AST 변환 로직 전체 보기 (Module.ts:161)](https://github.com/Han5991/fe-lab/blob/1707765c3eb6249234463033335d08927ca25a13/packages/@package/bundler/src/Module.ts#L161)**

---

> **💡 참고: ESM 번들링은 안 하나요?**
>
> 최신 라이브러리들은 **CJS**뿐만 아니라 **ESM** 포맷도 함께 지원하는 **Dual Package** 전략을 사용합니다. 하지만 우리는 번들러의 동작 원리(스코프 격리, 런타임 구현)를 깊이 이해하기 위해, 로직이 가장 명확하게 드러나는 **CJS 번들링** 구현에 집중했습니다.
...
생성된 `dist/index.js`를 열어보면, 수십 개의 파일이 하나의 **IIFE** 안에 질서 정연하게 들어있는 모습을 볼 수 있습니다. 우리는 브라우저 안에 **작은 Node.js**를 만들어낸 것입니다.
