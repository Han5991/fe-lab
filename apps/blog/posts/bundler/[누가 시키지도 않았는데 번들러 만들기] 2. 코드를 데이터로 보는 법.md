...
> "코드는 텍스트 Text가 아니다. 트리 Tree다."

1편에서는 번들러를 왜 만들어야 하는지, 그리고 `magic-string`이라는 도구를 맛봤습니다. 이제 본격적으로 코드를 분석하고, 여러 파일을 하나로 묶기 위한 준비를 시작해 봅시다.
...
번들러를 처음 만들 때 가장 먼저 마주하는 과제는 **"이 파일이 어떤 파일들을 필요로 하는가?"** 를 알아내는 것입니다. 즉, `import` 문을 찾아내야 합니다.

### 왜 정규표현식을 썼을까?

직관적으로 생각해보면, `import` 문은 꽤 단순한 패턴을 가지고 있습니다.
...
그래서 처음에는 **정규표현식**을 떠올리게 됩니다. 정규표현식은 강력하고, 빠르고, 무엇보다 **구현하기 쉽습니다.** 복잡한 파서 Parser를 도입하는 것보다 `String.match()` 한 방이면 끝날 것 같았죠.
...
정규표현식은 **문맥 Context**을 모릅니다. 주석인지, 문자열인지 구분하지 못하고 "패턴이 맞네? 가져와!"라고 외칠 뿐입니다. 결국 우리는 엉뚱한 파일을 import 하려고 시도하다가 에러를 뿜어내는 번들러를 마주하게 됩니다.

**결론**: 코드를 단순한 텍스트로 처리하려다간 큰코다친다. **구조**로 봐야 한다.
...
이 힌트를 가지고 실제 파일을 찾아내는 과정을 **Resolve 해결**라고 합니다. Node.js는 꽤 복잡한 Resolve 규칙을 가지고 있지만, 우리는 핵심적인 부분만 구현해 봅시다.
...
## 5. 하이라이트: "순환 참조 Circular Dependency 의 늪"

하지만 DFS에는 치명적인 약점이 있습니다. 바로 **순환 참조**입니다.

### A -> B -> A 무한 루프

만약 `a.js`가 `b.js`를 부르고, `b.js`가 다시 `a.js`를 부른다면 어떻게 될까요?

1. `createModule('a.js')` 호출
2. `a.js` 분석 -> `b.js` 발견
3. `createModule('b.js')` 호출
4. `b.js` 분석 -> `a.js` 발견
5. `createModule('a.js')` 호출... (**무한 반복**)
