...
excerpt: '수많은 소스 코드가 어떻게 하나의 결과물로 합쳐질까요? 매일 쓰면서도 정작 안을 들여다본 적 없던 번들러의 블랙박스를 열고, 라이브러리 없이 직접 손으로 구현해 보며 그 답을 찾아가는 시리즈를 시작합니다. 첫 편은 왜 이걸 만들기로 했는지에 대한 이야기입니다.'
thumbnail: 'bundler-prologue-thumb.png'
tags: ['bundler', 'library', 'build', 'tooling', 'javascript', 'architecture']
---

> 이전에 작성한 **[하루 만에 끝날 줄 알았던 디자인 시스템 배포가 3주 걸린 이유](https://blog.sangwook.dev/posts/npm-deploy-series-0-prologue/)** 에서는 "패키지를 배포하는 과정" 그 자체에 집중했습니다.  
...
### [Step 1. 개념과 도구](https://blog.sangwook.dev/posts/no-one-asked-library-bundler-01-concept/)

- 번들러의 역사적 맥락
- ESM이 번들링에 유리한 이유
- 코드 조작 도구 Magic String

### [Step 2. 그래프 그리기](https://blog.sangwook.dev/posts/no-one-asked-library-bundler-02-ast-graph/)

- 파일을 읽어 AST로 변환
- `import`를 추적해 의존성 그래프 구축
- 파일을 찾는 과정 구현

### [Step 3. 번들링과 스코프](https://blog.sangwook.dev/posts/no-one-asked-library-bundler-03-bundling-scope/)

- 그래프를 순회하며 코드 병합
- IIFE로 스코프 격리
- 브라우저용 `require` 함수 구현

### [Step 4. 완성도 높이기](https://blog.sangwook.dev/posts/no-one-asked-library-bundler-04-sourcemap/)
