...
소스맵까지 완성되었으니, 이제 번들러는 개발 경험(DX) 측면에서도 쓸 만한 도구가 되었습니다. 하지만 실전에서 쓰려면 한 가지 더 필요합니다.  
우리 라이브러리가 `react`나 `lodash`를 사용한다면, 번들 결과물에 React 전체 코드를 포함해야 할까요?

다음 편, **[Step 5. Externals와 번들 최적화]** 에서는:

- **Externals**: `react` 같은 외부 라이브러리를 번들에서 제외하는 전략
- **CJS/ESM Interop**: `import React from 'react'`가 CJS 환경에서 작동하는 방식
- 번들 최적화의 기타 고려 사항들
