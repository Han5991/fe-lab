...
```
DateRangeInput.test.tsx   34 tests   34.3s
DateInput.test.tsx        68 tests    5.1s
```

**테스트 수는 절반인데 시간은 7배.**

테스트 하나당으로 환산하면 이렇습니다.

| 파일                         | 테스트 수 |    총 시간 | 테스트 1개당 |
| :--------------------------- | --------: | ---------: | -----------: |
...
> 위 숫자(9.7ms / 463.4ms)는 원인을 눈으로 확인하려고 제가 따로 만든 재현 예제입니다.  
...
가장 쉬운 길은 `getByTestId`로 갈아타는 거였습니다. 하지만 그건 **접근성 쿼리를 포기하는 것**입니다.  
`getByRole`을 쓰는 이유가 있으니까요. 사용자가 화면을 인식하는 방식과 같은 방식으로 요소를 찾는 것 말입니다.
