...
이번 글은 조금 심심한 이야기입니다. 성능 개선 이야기처럼 26배나 47% 같은 숫자가 나오지 않습니다. 이 PR은 **아무것도 빨라지게 만들지 않았습니다.**
...
성능 숫자가 없는 PR이라고 했지만, 셀 수 있는 것들은 있습니다.

| 항목 | before | after |
| --- | --- | --- |
| `.github/` 안의 `node-version:` 리터럴 | 16개 | **0개** |
| Node 버전을 선언한 곳 | 0곳(로컬) / 16곳(CI) | **`.nvmrc` 1곳** |
| 3스텝 setup 프리앰블 | 14번 반복 | **composite action 1개** |
| docsite 가드 `if:` 줄 (3개 job 합계) | 9줄 | **3줄** |
| 문서가 안내하는 Node | 22 | **`.nvmrc`를 읽으라고 안내** |

파일 12개가 바뀌었고 96줄이 추가되고 134줄이 지워졌습니다. 순수하게 38줄이 줄었습니다. 새 파일 두 개(`.nvmrc`, composite action)를 **추가하고도** 저장소가 짧아진 겁니다.

숫자로 표현되지 않는 이득이 하나 더 있습니다. 다음에 Node를 26으로 올릴 때, 고쳐야 할 파일은 한 줄짜리 `.nvmrc` 하나입니다. 그리고 그 커밋의 diff는 누가 봐도 무엇을 바꿨는지 알 수 있습니다. 16개 파일에 흩어진 `24 → 26` 변경 diff와 비교하면, 리뷰 비용의 차이는 꽤 큽니다.
