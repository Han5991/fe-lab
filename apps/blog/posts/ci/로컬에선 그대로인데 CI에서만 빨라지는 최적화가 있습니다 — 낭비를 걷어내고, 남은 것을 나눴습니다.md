...
표를 세로가 아니라 가로로 읽어야 합니다. `test`는 두 실행에서 327초와 324초로 **거의 움직이지 않습니다**. 반면 build-sandbox는 385초에서 206초로 절반 가까이 내려갑니다. PR별 Next.js 캐시가 붙기 때문입니다(캐시 키를 PR별로 분리한 뒤 생긴 효과입니다. 시리즈 2편에서 다룹니다).
