...
세 run이 각각 역할이 다릅니다. main의 64초는 **캐시가 제대로 살아 있을 때의 기준선**이고, 대조군의 3.9분은 **캐시가 0일 때의 기준선**입니다. 환자를 그 사이에 놓고 어느 쪽에 붙는지 보는 겁니다. 둘 중 하나만 있으면 진단이 안 됩니다. 콜드만 있으면 "이 프로젝트는 원래 4분짜리군"으로 끝나고, main만 있으면 "PR은 뭔가 더 하니까 느리겠지"로 끝납니다.
...
### 이 고장이 다른 캐시로 번지지 않은 이유

여기까지 읽으면 "그럼 다른 캐시도 다 죽어 있는 것 아닌가" 싶어집니다. 그렇지는 않습니다. 이 버그는 세 조건이 **동시에** 성립해야 발동합니다.

| 조건 | `.next/cache` | 의존성 캐시(예: pnpm store) |
| --- | --- | --- |
| ① 산출물이 키에 없는 입력에 민감한가 | 예 (basePath) | 아니오 (lockfile이 전부) |
| ② 그 입력이 컨텍스트마다 다른가 | 예 (PR ≠ main) | 아니오 |
| ③ 소비자가 불일치를 감지해 폐기하는가 | 예 (config 지문) | — |

의존성 캐시가 안전한 건 운이 아니라 구조입니다. 내용이 content-addressed라 키가 헐거워도 잘못된 패키지를 집어올 수 없고, 일부만 맞으면 부족한 것만 더 받으면 됩니다. basePath 같은 빌드 시점 환경변수는 `node_modules`의 내용에 아무 영향이 없습니다.

위험한 쪽은 언제나 **빌드 산출물 캐시**입니다. 컴파일 결과는 그때의 config·환경변수·플래그에 통째로 물들어 있는데, 키는 보통 lockfile과 소스 해시만 담습니다. 그 틈이 이 버그의 서식지입니다.
...
정확히 하자면, Actions 캐시는 브랜치 스코프라 다른 PR이 구운 엔트리는 애초에 보이지 않습니다. 이 결정이 실제로 끊은 것은 **PR↔main 공유**고, PR↔PR은 원래 끊겨 있었습니다. 키 스코핑은 그 격리를 키 차원에서 한 번 더 명시한 셈입니다.
...
직관에 반하는 지점이 하나 있습니다. **고쳤더니 hit률이 떨어집니다.** 첫 푸시는 이제 반드시 miss니까요. 그 miss가 정상 동작의 증거라는 것을 미리 적어두지 않으면, 다음 사람이 "캐시가 안 먹네" 하고 되돌려 놓습니다.

### 택하지 않은 대안: 커밋 SHA를 물린 rolling 키

같은 자물쇠를 푸는 방법이 하나 더 있습니다. primary key 끝에 커밋 SHA를 붙이는 것입니다.

```yaml
key: nextjs-sandbox-${{ hashFiles('pnpm-lock.yaml') }}-${{ github.sha }}
restore-keys: |
  nextjs-sandbox-${{ hashFiles('pnpm-lock.yaml') }}-
```

이러면 primary key가 매 run 유일하므로 **hit이 구조적으로 날 수 없고**, 따라서 저장이 항상 실행됩니다. `Cache hit occurred on the primary key, not saving`이 발생할 수 없는 모양입니다. 복원은 `restore-keys`의 접두사 매칭이 담당해 직전 커밋 캐시를 물려받습니다. 캐시가 커밋마다 한 세대씩 굴러가는 셈입니다.

이 방식은 PR 스코프 키가 못 고치는 것도 하나 고칩니다. `.next/cache`는 append-only가 아니라 **매 빌드 갱신**되는데, `pr<n>` 키는 두 번째 푸시에서 또 primary hit이 나므로 저장이 다시 스킵됩니다. 즉 그 PR의 캐시는 **첫 푸시 시점에 얼어붙습니다.** 커밋이 쌓일수록 첫 푸시와의 거리가 벌어지니, 웜이긴 하되 점점 덜 웜해집니다.

그런데도 이 저장소에서는 쓸 수 없었습니다. `restore-keys`의 느슨한 폴백이 정확히 결정 1에서 거부한 것이기 때문입니다. basePath가 다른 캐시를 접두사 매칭으로 물려받으면, 지금 고치고 있는 그 버그가 폴백 경로로 되살아납니다.

| | 첫 푸시 | 저장 스킵 자물쇠 | 잘못된 캐시 복원 위험 |
| --- | --- | --- | --- |
| 공유 키 (이전 상태) | 콜드 | **있음** | 있음 |
| PR 스코프 키 (선택) | 콜드 | 없음(첫 푸시만) | 없음 |
| SHA rolling 키 | 웜 | 없음 | `restore-keys` 접두사 설계에 달림 |

세 번째를 안전하게 쓰려면 갈라야 하는 축을 `restore-keys` 접두사에도 전부 남겨야 합니다. lockfile 해시를 접두사에 포함시키면 의존성이 바뀐 캐시는 물려받지 않는 식으로요. 그런데 여기서 갈라야 하는 축은 basePath고, 그것을 접두사에 넣는 순간 PR별로 갈려서 결국 두 번째와 같아집니다. **basePath가 PR마다 다른 한, 첫 푸시는 어떤 키 설계로도 웜이 될 수 없습니다.** 이 결론은 뒤에서 다시 씁니다.
...
여기까지는 "죽은 캐시를 없애는" 이야기였습니다. "첫 빌드를 웜으로 만드는" 것은 다른 문제이고, 이 PR은 그것을 풀지 않았습니다. PR의 첫 푸시는 여전히 콜드입니다. 그건 고친 것이 아니라 **감수한** 것입니다.

제약이 하나 더 있습니다. GitHub Actions 캐시는 브랜치 스코프라, PR run이 읽을 수 있는 것은 자기 브랜치와 base·기본 브랜치의 엔트리뿐입니다. 다른 PR이 구운 캐시는 애초에 보이지 않습니다. 즉 **PR 첫 푸시를 덥힐 수 있는 시드는 main 캐시 하나뿐**입니다.

그러면 답은 키를 어떻게 짜느냐가 아니라 그 앞에 있습니다. **main 캐시가 PR 빌드에서도 유효하게 만드는 것** — 다시 말해 basePath를 빌드타임 입력에서 빼는 것입니다. 에셋을 상대 경로로 내보내거나, 프리뷰를 경로가 아니라 서브도메인으로 배포하면 모든 컨텍스트의 basePath가 같아집니다. 그러면 키를 가를 이유 자체가 사라지고, PR 첫 푸시도 main이 구운 캐시로 시작합니다.

키를 고치는 것은 증상 대응이고, 변하는 입력 자체를 없애는 것이 근본 처방입니다. 후자는 `ci.yml` 한 파일로 끝나지 않아 이번 범위 밖이었습니다. 다만 순서는 기억해 둘 만합니다 — **"캐시 키를 어떻게 짤까"를 고민하기 전에 "이 입력이 정말 달라야 하나"를 먼저 물어보는 것**입니다.
