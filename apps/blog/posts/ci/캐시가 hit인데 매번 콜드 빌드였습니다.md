...
세 run이 각각 역할이 다릅니다. main의 64초는 **캐시가 제대로 살아 있을 때의 기준선**이고, 대조군의 3.9분은 **캐시가 0일 때의 기준선**입니다. 문제의 PR run을 그 사이에 놓고 어느 쪽에 붙는지 보는 겁니다. 둘 중 하나만 있으면 진단이 안 됩니다. 콜드만 있으면 "이 프로젝트는 원래 4분짜리군"으로 끝나고, main만 있으면 "PR은 뭔가 더 하니까 느리겠지"로 끝납니다.

이제 질문이 명확해졌습니다. 로그는 hit이라는데, 왜 결과는 콜드와 같은가.

## 원인: 캐시 판정은 2층에서 일어납니다
...
머지 후 워밍 캐시가 걸린 재푸시 기준으로 PR CI 총 소요는 8분 56초에서 5분 10초가 됐습니다. 다만 이 숫자는 23분 간격으로 머지된 병렬화 PR(#3811)과의 **합산 효과**라, 캐시 수정만의 몫을 분리해 말할 수는 없습니다.
