...
| `emphasis`    | `false` | 핵심 경로. 선이 틸로 칠해지고 흐린 처리가 걷힌다                          |
| `arrow`       | `true`  | 화살촉. `arrow="false"` 면 선만                                           |

### 알아둘 규칙 네 가지

1. **엣지를 하나라도 쓰면 자동 연결이 꺼진다.** 하나도 안 쓰면 `row` 는 인접 노드끼리,
   `fan` 은 첫 노드에서 나머지로 자동 연결된다. 한 구간만 점선으로 바꾸려고 엣지 하나를
   추가하면 나머지 선이 전부 사라지므로, **쓸 거면 전부 쓴다.**
...
- **색은 2색뿐**: 회색(구조) + 틸(핵심 경로). 틸은 **그 글이 실제로 다루는 지점 하나**에만.
  노드 절반이 틸이면 강조가 아니라 배경색이다.
...
`status: draft` 인 글은 `/preview/{slug}/` 에서 볼 수 있다(dev 전용).
