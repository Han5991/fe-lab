...
test('RULES: 규칙은 정확히 29개', () => {
  ...
  // 테이블을 지나치지 못하게 한다.
  assert.equal(Object.keys(RULES).length, 29);
});
...
test('RULES: 전체 집합을 봐야 하는 규칙은 corpus 계열 scope 둘뿐', () => {
  const corpusRules = Object.entries(RULES)
    ...
    .map(([id]) => id)
    .sort();
  assert.deepEqual(corpusRules, ['duplicate-description', 'duplicate-slug']);
  // 가시성 필터까지 걸리는 쪽은 duplicate-description이다 — 공개 전 예약 글을
  // 비교에 섞으면 산출물에 없는 충돌로 빌드가 막힌다(corpus.ts 참고).
  assert.equal(RULES['duplicate-description'].scope, 'corpusVisible');
  assert.equal(RULES['duplicate-slug'].scope, 'corpus');
});
...
test('RULES: isPostFile 게이트(scope=post)가 걸린 규칙', () => {
  ...
  const postScoped = Object.entries(RULES)
    ...
    .map(([id]) => id)
    .sort();
  assert.deepEqual(postScoped, ['body-h1', 'missing-image-alt']);
});
...
test('resolveSeverity: 센티널 규칙은 strict + 발행 대상일 때만 에러', () => {
  ...
  assert.equal(resolveSeverity(rule, PUBLISHED, {}), 'warning');
  // strict + 발행 → 에러
  assert.equal(resolveSeverity(rule, PUBLISHED, { strict: true }), 'error');
  ...
  assert.equal(
    resolveSeverity(rule, { title: 'x' }, { strict: true }),
    'warning',
  );
});
...
test('resolveSeverity: 고정 심각도 규칙은 strict와 무관하게 테이블 값 그대로', () => {
  assert.equal(resolveSeverity('missing-title', PUBLISHED, {}), 'error');
  assert.equal(
    resolveSeverity('missing-title', { title: 'x' }, { strict: true }),
    'error',
  );
  assert.equal(resolveSeverity('duplicate-tags', PUBLISHED, {}), 'warning');
  assert.equal(
    resolveSeverity('duplicate-tags', PUBLISHED, { strict: true }),
    'warning',
  );
});
