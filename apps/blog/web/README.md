...
| `platform`          | `src/lib/platform`       | `shared`, 외부 `@supabase/supabase-js`·`@supabase/postgrest-js`                                                                                                                              |
| `analytics`         | `src/domain/analytics`   | `shared`, `platform`, `content-pkg`(날짜 유틸 등)                                                                                                                                            |
...
| 스크립트                   | 하는 일                                                                                                                                                                                                                                                                  |
| :------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pnpm dev`                 | `supabase start`(Docker) → `next dev`. `predev:web`이 먼저 `NODE_ENV=development blog-content build`(경고 수준, draft·예약 글 몫까지)를 돌린다. Next만 띄우려면 `pnpm dev:web`                                                                                           |
| `pnpm build`               | `prebuild`(`blog-content build --strict`) → `next build` → `check-seo` → `check-bundle`. **이 넷이 한 덩어리** — CI·배포도 이 스크립트 하나를 부른다. 넷 모두 먼저 내보낸 기준 시각 하나(`BLOG_CONTENT_NOW`, 이미 있으면 그 값)로 예약 글 공개를 판정한다                |
