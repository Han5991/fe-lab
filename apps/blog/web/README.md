...
│  ├─ domain/analytics/ analytics 레이어 — @blog/analytics에 클라이언트를 꽂는 배선 배럴 2개(index · admin)
│  ├─ domain/auth/      auth 레이어 — 세션 배럴(@blog/analytics의 AuthRepository에 client.auth 주입) + 관리자 이메일 판정(adminAccess)
│  ├─ lib/platform/     platform 레이어 — Supabase 클라이언트 생성 둘(client · publicClient). env를 읽는 자리라 앱에 남았다
...
| `shared`            | `src/shared`             | `content-pkg`만(라우트 계약 `encodePostSlug`·`POSTS_PATH`) — 앱 내부 어디에도 기대지 않는 최하단. 라우트 경로·전환 네임스페이스의 단일 출처                                                  |
| `platform`          | `src/lib/platform`       | `shared`, `analytics-pkg`(타입), 외부 `@supabase/supabase-js`·`@supabase/postgrest-js`                                                                                                       |
| `analytics`         | `src/domain/analytics`   | `shared`, `platform`, `content-pkg`, `analytics-pkg`(계산·계약·저장소 팩토리)                                                                                                                |
...
- **조회수** — `useViewCount` → `@blog/content`의 `viewCookie`(6시간 쿨다운, RPC 전에 쿠키를 먼저 심어 두 탭 레이스 방지) → `src/domain/analytics` 배럴의 `incrementViewCount`(= `@blog/analytics`의 저장소에 `src/lib/platform/publicClient`(PostgREST-only 경량 클라이언트)를 꽂은 것)로 `increment_view_count` RPC.
- **Admin** — `AdminLayoutClient` → `AdminGuard`(세션 `useSuspenseQuery`, dev에서만 bypass) → React Query 훅 → `@/src/domain/analytics/admin` 배럴 → `@blog/analytics`의 `createAdminAnalytics` → 같은 패키지의 `AdminApiClient` → Supabase Edge Function `admin-analytics`(호출자 JWT를 `ADMIN_EMAIL`과 대조 후 `service_role`로 RPC). 세션·로그인·로그아웃과 관리자 이메일 판정은 전부 `@/src/domain/auth` 배럴 경유 — 가드·로그인 페이지가 `client.auth`를 직접 만지지 않는다. 로그인 **경로** 계약(`ADMIN_LOGIN_PATH`·`isAdminLoginPath`…)은 `@/src/shared/routes`가 단일 출처다. 글 목록은 빌드가 만든 `/admin-posts-index.json`(`getAdminPostsIndex` 한 곳이 읽는다). `src/domain/analytics/index.ts`와 `admin.ts`를 **일부러 두 배럴**로 나눠 공개 페이지가 auth 세션 supabase-js를 끌고 오지 않게 한다.
...
- **PR 프리뷰**: `.github/workflows/preview-blog.yml` — `wrangler versions upload`로 버전만 올리고(트래픽 이동 없음) 브랜치 고정 alias URL과 커밋별 URL을 PR에 코멘트한다. 앱 둘을 매트릭스로 돌리므로 SvelteKit 판(`apps/blog/web-svelte`) 프리뷰도 같은 워크플로가 자기 Worker에 낸다. 줄마다 자기 앱 디렉터리나 두 앱이 함께 읽는 경로(원고·`packages/@blog`·워크플로 파일)가 바뀐 PR에서만 빌드한다. Vercel은 더 이상 쓰지 않는다.
