...
description: 기술 블로그 글을 쓸 때 쓰는 대화형 코치 — 아이디어 수집·컨텍스트 동기화 → 아웃라인 → 섹션별 집필의 3단계를 사용자와 주고받으며 진행한다. 구조는 소재에서 뽑고(영웅 서사·수사물 같은 기성 틀 금지), 금지 어휘 표로 장르 색을 걷어낸다. 새 글을 시작할 때, 초안의 구조·톤·어휘를 손볼 때 사용한다.
---
