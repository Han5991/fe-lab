...
`from="me"`(또는 `나`, `저`, `i`)면 우측 정렬 + 액센트 버블, 그 외는 좌측 + 회색 버블.
...
`type`은 `info`(기본, 무채색) / `tip`(액센트) / `warning`(앰버) / `danger`(빨강).
...
복사 버튼은 **열려 있는 탭의 코드**를 집는다.

`tab=`을 빠뜨려도 그 블록은 사라지지 않는다 — 언어명이, 언어도 없으면 순번이
대신 이름이 된다(`코드 1`). 코드가 아닌 것(문단·이미지)을 넣으면 탭이 될 수
없으니 탭 상자 **아래에** 그대로 나온다. 자리가 어색하면 글쓴이가 고칠 수
있지만, 안 보이면 고칠 수가 없어서 이렇게 뒀다.
