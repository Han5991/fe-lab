...
| `apps/blog/web/content.config.mts`          | **경로 앵커** — `defineContent({ root: import.meta.url })` default export. 이 파일의 위치가 `dirs.*`의 기준이고, CLI(cwd walk-up)와 앱(`src/content.ts` 정적 import)이 함께 읽는 단일 출처                                                                                    |
...
| `…content/src/shared/contentConfig.ts`      | `defineContent({...})` 설정 표면 — 서버·빌드 전용, **`root`만 필수**. 클라이언트가 소비하는 리터럴은 `contentValues.ts`(값-only 모듈, `constants.ts`가 재수출)가 갖고, 설정은 그 값을 기본값으로 소비한다. 절대 경로 해석은 `contentPaths.ts`의 `resolveContentPaths(config)` |
