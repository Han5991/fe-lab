...
| `apps/blog/redirect/`                       | apex 리다이렉트 Worker — `sangwook.dev`·`www.sangwook.dev` → `blog.sangwook.dev`(둘 다 `routes`에 `custom_domain`으로 선언). Vercel의 `apps/next.js` `redirects()`를 순수 함수로 옮긴 것. 파일 경로에 후행 슬래시를 붙이지 않는 판정이 핵심이고(과거 `sitemap.xml` 6개월 404의 원인) `src/redirect.test.ts`가 308·쿼리스트링과 함께 잠근다                                                                                                                                                                              |
