...
| `/list-good-prs` | 머지 가능 + 리뷰 통과 PR 목록 (읽기 전용)                                  |
...
규칙은 ① path가 `/`나 **알려진 확장자 31종**으로 끝나면 그대로 넘기고 ② 나머지는 후행
슬래시를 붙여 넘긴다(둘 다 308, 쿼리 보존). 예전엔 전용 Worker가 하던 일이다.
**확장자 allowlist인 이유**는 이 블로그의 슬러그 때문이다 — "마지막 세그먼트에 점이
있으면 파일"이라는 정규식 방식이 더 일반적으로 보이지만, `vue-3.0`·`next-15.2` 같은
버전 번호 슬러그를 파일로 오분류해 후행 슬래시를 빼앗고 404를 만든다. allowlist는 그
오분류가 구조적으로 불가능하다. 대신 **목록에 없는 확장자는 조용히 404가 되므로**
(2026-09-05에 `site.webmanifest`가 실제로 그랬다) 새 종류의 파일을 내보내기 시작하면
목록에 추가할 것. 회귀는 `site-smoke-collect.py`의 apex probe가 잡는다.
...
- **`@blog/content` 내부**: `shared`(node 코어만) → `post`(+gray-matter) → `seo`(순수 계산) →
  ...
  단계를 동적 import로 든다). 밖으로 여는 문은 넷이다 — `@blog/content`·`@blog/content/seo`에
  더해 **클라이언트용** 순수 leaf 문 둘(`@blog/content/urls`·`@blog/content/dates`). 뒤의 둘이 있는 이유는
  번들러다: 배럴은 `node:fs`를 함께 여는데 그것을 클라이언트 그래프에서 열어도 안전한지는
  Next의 `optimizePackageImports`가 정해 주던 것이라, 대응물이 없는 Vite에서 `postPath`
  하나를 배럴로 들이자 fs가 통째로 externalize됐다(`packages/@blog/content/src/post/urls.ts` 주석).
...
- **`trailingSlash: true`**: 후행 슬래시 설정. 호스팅 쪽 짝은 `wrangler.jsonc`의 **`html_handling: force-trailing-slash`**다 — 기본값(`auto-trailing-slash`)은 슬래시 유무 양쪽에 200을 주어 정규 URL이 둘로 갈라진다. **`skipTrailingSlashRedirect: true`가 짝이다** — 없으면 next/link가 `.`이 든 slug(`turborepo-next.js-docker`)를 파일로 보고 붙인 슬래시를 도로 벗겨 링크가 리다이렉트를 한 번 더 탄다(지금 그 코드는 **307**이다 — 상태 코드의 단일 출처는 `wrangler.jsonc`다). 그래서 내부 href는 전부 스스로 후행 슬래시를 달아야 하고(`postPath`·`archivePath`가 그렇게 한다), 산출물은 `check-seo`의 `link-trailing-slash` 규칙이 지킨다
...
| `<diagram>` + frontmatter `hero:`             | 구조 그림. 저작법 전체는 **`blog-diagrams` 스킬** — 요약은 `blog-components` 스킬                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
수치와 그 근거, 그리고 **글 자체의 구조·톤·어휘**는 아래 네 스킬이 단일 출처이고,
이 파일에 다시 적지 않는다.
...
| 구조 그림 — 선언형 태그 prop 표, 노드 개수·폭 감각, `hero:` 슬롯, 새 다이어그램 코드 등록                                                        | `blog-diagrams`      |
...
- 다이어그램 저작 가이드 전문: `blog-diagrams` 스킬
...
| `packages/@blog/analytics`                   | 조회수·대시보드 도메인 패키지 — 순수 계산(개요·파생 통계)·계약(`AdminApi`·`AuthApi`)·저장소 팩토리·`database.types.ts`(`gen:types` 산출물)·Edge Function과 공유하는 `adminActions.ts`. **클라이언트를 만들지 않는다**(주입받는다) — `@supabase/*`는 타입으로만 쓰고, 값으로 여는 것은 `@blog/content/dates` 하나다. 앱은 `src/domain/{analytics,auth}` 배럴로만 연다. 내부는 `packages/@blog/analytics/README.md`                                                                                                                                                                                                                                                                                                                                                                                        |
| `packages/@blog/content`                     | 콘텐츠 프레임워크 패키지 — 스키마·로더·공개 판정·URL 계약·빌드 스크립트·2층 검증. 문 넷(`@blog/content` + `@blog/content/seo` + 클라이언트용 `@blog/content/urls`·`/dates`) + `bin`의 `blog-content`, 소스 익스포트(빌드 스텝 없음). 내부는 `packages/@blog/content/README.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `apps/blog/web/design/redesign-decisions.md` | 리뉴얼 결정 기록 — 무엇을 왜 그렇게 정했는지만 답한다. 현행 수치의 출처가 아니다(그건 `blog-preset.ts`와 `blog-design-system` 스킬)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
