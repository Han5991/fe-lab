...
| `pnpm check-bundle`                           | 산출물(`out/`) JS 청크의 admin 코드 누수 검사 — 공개 페이지가 도달하는 청크(HTML 참조 + 청크 간 지연 로드 폐포)에 admin 마커(`content.values.mts`의 `BUNDLE_GUARD_MARKERS`)가 있으면 실패. **admin 청크에 마커가 없어도 실패다**(양성 대조 — 번들러 출력이 바뀌어 마커가 죽으면 검사 무력화를 알린다). `check-seo` 다음, `pnpm build`의 마지막 단계 |
...
| `…content/src/scripts/check-bundle.ts`      | 빌드 산출물 번들 누수 검사 (CI 게이트) — 마커 선언은 앱 `content.values.mts`의 `BUNDLE_GUARD_MARKERS`, 계약은 `contentConfig.ts`의 `BundleGuardsConfig`                                                                                                                                                                                                                                                                                                                                                      |
