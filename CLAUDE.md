...
| `pnpm check-bundle`                           | 산출물(`out/`) JS 청크의 admin 코드 누수 검사 — 공개 페이지가 도달하는 청크(HTML 참조 + 청크 간 지연 로드 폐포)에 두 계열을 본다 — ① admin 전용 코드가 공개 도달 청크에 있으면 실패, ② 서버 전용 값(llms 산문 카나리아)이 어떤 청크·페이지에든 있으면 실패. 두 계열 다 양성 대조를 가진다(admin 마커는 admin 청크에, 서버 전용 마커는 앵커 산출물에 반드시 존재 — 마커가 죽으면 검사 무력화를 알린다). 선언은 `content.values.mts`가 통째로 소유. `check-seo` 다음, `pnpm build`의 마지막 단계 |
...
| `…content/src/scripts/check-bundle.ts`      | 빌드 산출물 번들 누수 검사 (CI 게이트) — 두 계열(admin: `ADMIN_PATH_PREFIX`+`BUNDLE_GUARD_MARKERS` · 서버 전용: `SERVER_ONLY_MARKERS`)의 선언은 통째로 앱 `content.values.mts` 소유, 계약은 `contentConfig.ts`의 `BundleGuardsConfig`. 패키지에 기본값 없음 — 미선언 사이트는 검사 스킵                                                                                                                                                                                                                      |
...
| `…content/src/shared/contentConfig.ts`      | `defineContent({...})` 설정 표면 — 서버·빌드 전용. **사이트 고유 값에는 기본값이 없다**(`ContentValues` 계약: `root`·`site`·`author`·`timezone`·`registries.diagramNames`·`og.palette`가 필수). 남은 기본값은 어떤 사이트에서도 같은 값뿐이고(SEO 예산·펜스 라벨·경로 관례·OG 규격), 나머지는 비었거나(sitemap 정적 페이지·우선순위·llms facts) 파생된다(llms 소개 산문 ← `site.description`). 값은 앱의 `content.values.mts`가 소유한다. 절대 경로 해석은 `contentPaths.ts`의 `resolveContentPaths(config)` |
